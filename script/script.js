...
// Gère : authentification (cookies JWT), rôles, panier (localStorage),
//        compteur panier, toasts, formatage des prix.
//
// MIGRATION API : les fonctions de gestion des comptes et commandes
// en localStorage ont été supprimées. Elles sont remplacées par les
// appels API via le module api.js. Le panier reste en localStorage
// (panier anonyme avant validation de la commande côté serveur).
// =======================================================
...
// Clé du panier dans le localStorage (reste en local tant qu'on n'a pas commandé)
...
/**
 * @description Récupère la valeur d'un cookie par son nom
...
/**
 * @description Crée (ou met à jour) un cookie
 * @param {string} name  - Nom du cookie
...
/**
 * @description Supprime un cookie (en le vidant avec une date d'expiration passée)
 * @param {string} name
 */
...
// On ne VÉRIFIE PAS la signature côté front (c'est le rôle du backend).
// On décode juste le payload pour extraire email, rôles, exp.
// =======================================================

/**
 * @description Décode la partie payload d'un JWT sans vérifier la signature.
 * La vérification est faite par le backend à chaque requête authentifiée.
 * @param {string} token
 * @returns {Object|null}
 */
export function decodeJwtPayload(token) {
    ...
    try {
        // Base64 URL-safe → Base64 standard
...
/**
 * @description Vérifie que le JWT est présent et valide (signature + expiration).
 * @param {string} token
 * @returns {boolean}
 */
...
/**
 * @description Vérifie si l'utilisateur est connecté (JWT présent et non expiré)
 * @returns {boolean}
 */
...
/**
 * @description Récupère le rôle principal ("client" ou "admin") depuis le JWT Symfony.
 * Le backend Symfony renvoie des rôles du type ['ROLE_USER', 'ROLE_ADMIN']
 * dans le payload du JWT. On transpose ici vers notre convention front.
 * @returns {string|null}
 */
...
/**
 * @description Récupère l'email de l'utilisateur connecté depuis le JWT.
 * Symfony Lexik JWT stocke l'email dans le champ "username" par défaut.
 * @returns {string|null}
 */
export function getEmail() {
		...
		// Lexik JWT : le champ par défaut est "username"
...
export function showAndHideElementsForRole() {
	...
	for (const el of allElements) {
		...
		switch (el.dataset.show) {
			case 'disconnected':
				// Masqué si connecté (lien "Connexion")
				if (userConnected) el.classList.add("d-none");
				break;
			case 'connected':
				// Masqué si déconnecté (lien "Mon compte", bouton déconnexion)
				if (!userConnected) el.classList.add("d-none");
				break;
			case 'admin':
...
/**
 * @description Enregistre le panier dans localStorage et met à jour le compteur
 * @param {Array} panier
 */
...
/**
 * @description Ajoute un produit au panier (ou incrémente sa quantité si déjà présent)
...
/**
 * @description Formate un prix en euros au format français
 * Accepte un number OU une string (l'API renvoie les decimals en string)
...
/**
 * @description Affiche un toast Bootstrap temporaire en bas à droite
...
export function afficherToast(message, type = 'success') {
    ...
    const icone = type === 'danger' ? 'bi-exclamation-triangle-fill'
    ...
    toastEl.innerHTML = `
        <div class="d-flex">
            <div class="toast-body">
                <i class="bi ${icone}" aria-hidden="true"></i> ${sanitizeString(message)}
            </div>
